...
function detectFormat (txt, alrt) {
  ...
  var id= enc? enc[0]: 0;
  if (!alrt) return id;
  if (!id) alert("Xin lỗi, VIETUNI không thành công...");
  else alert("Bảng mã được dùng la`: "+ enc[1]);
  return id;
}
...
this.register= function (constructor) {
  ...
  this.patterns[this.length++]= new RegExp(re, map.maxchrlen?'gi':'g');
}

this.getMap= function(id) {
  var ind= this.length-1;
  if (typeof(id)=='number') ind= id;
  else while((ind>0) && (this.names[ind]!=id)) --ind;
